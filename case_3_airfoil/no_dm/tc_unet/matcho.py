...
class Unet2D(nn.Module):
    def __init__(
        ...
        for ind, (dim_in, dim_out) in enumerate(in_out):
            ...
            self.downs.append(nn.ModuleList([
                ...
                block_klass(dim_in, dim_in, time_emb_dim = time_dim),
                Residual(PreNorm(dim_in, LinearAttention(dim_in, heads=attention_heads, dim_head=dim_head))),
        ...
        for ind, (dim_in, dim_out) in enumerate(reversed(in_out)):
            ...
            self.ups.append(nn.ModuleList([
                ...
                block_klass(dim_out + dim_in, dim_out, time_emb_dim = time_dim),
                Residual(PreNorm(dim_out, LinearAttention(dim_out, heads=attention_heads, dim_head=dim_head))),
                Upsample(dim_out, dim_in) if not is_last else  nn.Conv2d(dim_out, dim_in, 3, padding = 1)
            ]))

        default_out_dim = channels * (1 if not learned_variance else 2)
        self.out_dim = self.Par["nf"] #default(out_dim, default_out_dim)
    ...
    def forward(self, x, time, x_self_cond = None, use_grid=True):
        ...
        # if self.channels == self.Par["lb"] + 2 and use_grid:
...
if __name__ == "__main__":
    ...
    pred = model(torch.rand((16, 3, 64, 64)), time=torch.rand((16, )))
    print('OK')