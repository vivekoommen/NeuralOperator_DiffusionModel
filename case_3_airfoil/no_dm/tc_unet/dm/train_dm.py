...
# colors = ["black","white", "blue", "purple", "red", "yellow", "green", "grey", "violet", "lavender"]
# cmap_name = "temp"
...
# x_train = np.load(f"../data/{temp}M_HFS_train_pred.npy")
# y_train = np.load(f"../data/train_GT.npy")

# x_val = np.load(f"../data/{temp}M_HFS_val_pred.npy")
# y_val = np.load(f"../data/val_GT.npy")

# x_test = np.load(f"../data/{temp}M_HFS_val_pred.npy")
# y_test = np.load(f"../data/val_GT.npy")
...
for epoch in range(num_epochs):
    ...
    if epoch != 0 and epoch % 10 == 0:
        ...
        with torch.no_grad():
            for l_fidel, h_fidel in val_loader:
                with autocast():
                    ...
                    loss, f_err, r_err = error_metric(l_fidel.to(device), pred, h_fidel.to(device), epoch, Par)
        ...
        print(time_stamp + f' - Epoch {epoch + 1}/{num_epochs}, Train Loss: {train_loss:.4e}, Val Loss (Spectrum): {val_loss:.4e}, Val Loss (field): {field_err:.4e}, best model: {best_model_id}, best err: {best_f_loss:.4e}, ref err: {ref_err:.4e}, LR: {scheduler.get_last_lr()[0]:.4e}, train time: {train_time:.2f}, val time: {val_time:.2f}')
...
with torch.no_grad():
    for l_fidel, h_fidel in test_loader:
        with autocast():
            pred = model.sample(l_fidel.to(device))
            loss   = error_metric(l_fidel.to(device), pred, h_fidel.to(device), epoch, Par, is_plot=False)
        test_loss += loss.item()
