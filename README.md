# NO+DM

This repository contains the code accompanying our paper, ["Integrating Neural Operators with Diffusion Models Improves Spectral Representation in Turbulence Modeling"](https://arxiv.org/pdf/2409.08477), accepted in *Proceedings of the Royal Society A*. Our work demonstrates how diffusion models can help mitigate the spectral bias suffered by neural operators. Below, you will find the structure, usage instructions, and dependencies for this repository.

---

## Repository Structure

- **`case_1_kolmogorov/`**: Contains code and data access instructions for the Kolmogorov flow case.
- **`case_2_buoyancy/`**: Contains code and data access instructions for the buoyancy-driven smoke evolution case.
- **`case_3_airfoil/`**: Contains code and data access instructions for the airfoil simulation case.
- **`case_4_LES_jet/`**: Contains code and data access instructions for the turbulent subsonic jet case.
- **`case_5_schlieren_jet/`**: Contains code and data access instructions for the schlieren turbulent jet visualization case.
- **`requirements.txt`**: Minimal dependencies required to run the code.
- **`requirements_full.txt`**: Full dependencies for all features and cases.

---

## Diffusion-Corrected Autoregressive (DCAR) Rollouts

We found that DCAR rollouts help stabilize longer forecasts. Checkout our implementation `case_3_airfoil/no_dm/tc_unet/dm/dcar_rollout.ipynb`

---

## Installation

1. Clone this repository:
   ```bash
   git clone https://github.com/vivekoommen/NeuralOperator_DiffusionModel.git
   cd NeuralOperator_DiffusionModel
   ```

2. Create a virtual environment and install dependencies:
   ```bash
   python3 -m venv venv
   source venv/bin/activate  # On Windows, use `venv\Scripts\activate`
   pip install -r requirements.txt
   ```

3. For full features, install:
   ```bash
   pip install -r requirements_full.txt
   ```

---

## Usage

### Dataset Preparation

Each case folder contains a `data/` subdirectory with scripts for dataset preparation. The datasets can be accessed or generated as follows:

| Case                              | Dataset Link                                                                                                              |
|-----------------------------------|--------------------------------------------------------------------------------------------------------------------------|
| **Kolmogorov Flow**               | Dataset is generated by the notebook in `case_1_kolmogorov/data/ns_data_generator.ipynb`. Also available for direct download on Zenedo.       |
| **Buoyancy-Driven Flow**          | [link](https://huggingface.co/datasets/pdearena/NavierStokes-2D/tree/main) |
| **LES of Turbulent Airfoil**      | airfoilLES_snapshots_midspan.zip from [link](https://deepblue.lib.umich.edu/data/concern/data_sets/3n203z458?locale=en)            |
| **LES of Turbulent Jet**          | jet_3D_01000.zip from [link](https://deepblue.lib.umich.edu/data/concern/data_sets/2b88qc54w?locale=en)                             |
| **Schlieren Velocimetry of Jet**     | 20210125-Run1.zip from [link](https://zenodo.org/records/6136052)                                                                  |

### General Workflow

1. **Train the Neural Operator:**
   Navigate to the desired case folder and run:
   ```bash
   python3 -u train_no.py
   ```

2. **Prepare Data for Diffusion Model:**
   Open the Jupyter Notebook `no_postprocess.ipynb` in the same folder and execute it.

3. **Train the Diffusion Model:**
   Navigate to the `dm/` folder inside the case directory and run:
   ```bash
   python3 -u train_dm.py
   ```

4. **Post-process Results:**
   Open the Jupyter Notebook `dm_postprocess.ipynb` to generate figures and analyze results.

---

## Results

Run the workflow described above to reproduce the results and generate the figures presented in the paper. Outputs include:
- Performance metrics.
- Visualization of the physical phenomena for each case.

---

## Citing This Work

If you use this code, please cite our paper:

```bibtex
@article{oommen2024integrating,
  title={Integrating neural operators with diffusion models improves spectral representation in turbulence modeling},
  author={Oommen, Vivek and Bora, Aniruddha and Zhang, Zhen and Karniadakis, George Em},
  journal={arXiv preprint arXiv:2409.08477},
  year={2024}
}
```

---

## License

This project is licensed under the MIT License. See the `LICENSE` file for details.

---

## Acknowledgments

- The Kolmogorov flow was simulated using the publicly available pseudo-spectral solver provided in [FNO's Repository](https://github.com/neuraloperator/physics_informed/blob/d1835d1e6ee9d7969455ceb36040389a23d04d85/solver/legacy_solver.py#L68)
- This repository builds upon the diffusion model implementation by [Phil Wang](https://github.com/lucidrains/denoising-diffusion-pytorch/tree/main/denoising_diffusion_pytorch).
